...
class MemoryFrame:
    def __init__(self, header, adc_channels):
        self.header = header
        self.adc_channels = adc_channels  # cells is a list of 1024 tuples of 8 elements each
    ...
    def parse(data):
        ...
        transposed_cells = list(zip(*cells))  # Transpose rows to columns (8 lists of 1024 elements)
...
if __name__ == '__main__':
    ...
    if os.path.exists(file_path):
        ...
        print(f"First Frame Cells (transposed): {frames[0].adc_channels[:5]}")  # Printing only the first 5 arrays
    else:
