...
from calculation_scripts.calculations import (calculate_amplitude,
                                              calculate_fwhm)
...
class App:
    def __init__(self, root):
        ...
        self.select_file_btn = tk.Button(
            self.root, text="Select File", command=self.select_file, width=10
        )
        self.parse_file_btn = tk.Button(
            self.root,
            text="Parse File",
        ...
        self.select_file_btn.grid(
            row=0, column=0, padx=5, pady=5, sticky="ew", columnspan=1
        )
        self.parse_file_btn.grid(
            row=1, column=0, padx=5, pady=5, sticky="ew", columnspan=1
        )

        self.plot_graph_btn = tk.Button(
            self.root,
            text="Plot Graph",
        ...
        self.plot_graph_btn.grid(
            row=0, column=1, padx=5, pady=10, sticky="ew", columnspan=1
        )
        ...
        self.average_btn = tk.Button(
            self.root,
            text="Plot average signals",
            command=self.plot_average_signal,
            state=tk.DISABLED,
            width=10,
        )
        self.average_btn.grid(row=0, column=4, padx=5, pady=5, sticky="ew")

        self.amplitude_label = tk.Label(
            self.root, text="Signal amplitudes: ", anchor="w"
        )
        ...
        self.canvas.get_tk_widget().grid(
            row=5, column=0, columnspan=6, pady=20, sticky="nsew"
        )
    ...
    def parse_file(self):
        ...
        try:
            ...
            self.frames = frames
            messagebox.showinfo(
                "Success", f"Parsing complete. Number of frames: {len(frames)}"
            )
    ...
    def plot_graph(self):
        ...
        if not frame_number.isdigit():
            messagebox.showerror(
                "Invalid Input", "Please enter a valid number for frame"
            )
            return
        ...
        if frame_number < 0 or frame_number >= len(self.frames):
            messagebox.showerror(
                "Invalid Frame",
                f"Frame number should be between 0 and {len(self.frames) - 1}",
            )
            return
        ...
        try:
            ...
            messagebox.showerror(
                "Invalid Input",
                "Please enter valid numbers for channels, separated by commas",
            )
            return
        ...
        invalid_channels = [
            ...
            if ch < 0 or ch >= len(self.frames[frame_number].adc_channels)
        ]
        if invalid_channels:
            messagebox.showerror(
                "Invalid Channel",
                f"Invalid channels: {', '.join(map(str, invalid_channels))}",
            )
            return
    ...
    def plot_average_signal(self, adc_timestep: float = 0.2):
        ...
        for ch_number in ch_numbers:
            if ch_number < 0 or ch_number >= len(self.frames[0].adc_channels):
                messagebox.showerror(
                    "Invalid Channel", f"Channel {ch_number} is out of range"
                )
                return
            ...
            for frame in self.frames[:500]:
                ...
                summed_data = [
                    sum_value + channel_data[i]
                    for i, sum_value in enumerate(summed_data)
                ]
                frame_count += 1

            averaged_data[ch_number] = [
                sum_value / frame_count for sum_value in summed_data
            ]
        ...
        amplitude_text = "; ".join(
            [f"Ch {ch}: {amplitudes[ch]:.2f}" for ch in ch_numbers]
        )
